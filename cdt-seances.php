...
//// HTML ////
//////////////
debut($mysqli,"Cahier de texte en ${matiere['nom']} - Modifier les types de séances",$message,5,"cdt?$cle&seances",array('action'=>'cdt-types','matiere'=>$mid,'css'=>'datetimepicker'));
echo <<<FIN
  ...
  <article>
    ...
    <input onclick="location.href='cdt?$cle&amp;raccourcis'" type="button" class="ligne" value="Modifier les boutons de raccourcis">
  </article>
...
';
// Récupération
$resultat = $mysqli->query("SELECT id, ordre, titre, cle, deb_fin_pour, nb FROM `cdt-types` WHERE matiere = $mid ORDER BY ordre");
$mysqli->close();
...
?>
  ...
  <form id="form-ajoute" data-action="ajout-cdt-type">
